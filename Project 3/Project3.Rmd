...
```{r showlistings, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE, paged.print=TRUE}
#Minor clean-up
listings$description <- str_extract(listings$description,".+") 

drv <- dbDriver("PostgreSQL")
con <- dbConnect(drv, user='postgres', password='password', dbname='indeed', host='host')
...
#Query to all job listings from SQL database.
sql <- " 
...
```{r hidden, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
drv <- dbDriver("PostgreSQL")
con <- dbConnect(drv, user='postgres', password='kusya5341', dbname='indeed', host='localhost')
